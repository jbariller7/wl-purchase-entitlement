...
- Responsive `/account/` customer interface for sign-in/linking, effective access, checkout, provider-specific subscription management, legacy claims, PC/Mac approval, session revocation and account deletion.
- Responsive `/admin/` operations console with customer lookup, manual grants, two-step refunds and regional price changes, dry-run imports, delivery retries, key inventory, provider monitoring and audit history. Explicit `?demo=1` sessions are stateful but fictional and side-effect-free; the normal page never substitutes demo records for unavailable services.
