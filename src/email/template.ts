...
export function renderConfirmation(input:Confirmation) {
 ...
 if(!isMobile && !input.renewal){
  if(!input.keys.length)throw new Error('Delivery is not ready.');
  add(delivery==='steam'?c[6]:c[7]);
