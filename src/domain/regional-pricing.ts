...
export function currencyFractionDigits(currency: string): 0 | 2 | 3 {
  const code = currency.toUpperCase();
  return ZERO_DECIMAL_CURRENCIES.has(code) ? 0 : code === "KWD" ? 3 : 2;
}
...
export function stripeMinorAmount(currency: string, majorAmount: string): number {
  ...
  return Math.round(value * 10 ** currencyFractionDigits(normalizedCurrency));
}
...
export function stripeMajorAmount(currency: string, minorAmount: number): string {
  ...
  return (minorAmount / 10 ** digits).toFixed(digits);
}
