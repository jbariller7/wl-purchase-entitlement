...
function renderCustomers() {
  ...
  const paymentRows = (c?.payments || []).map((p) => [formatDate(p.createdAt), `${p.id} (${p.livemode ? "LIVE" : "TEST"})`, formatMoney(p.amountReceived || p.amount, p.currency), formatMoney(p.amountRefunded || 0, p.currency), p.status, p.refundableAmount > 0 ? htmlCell(`<button class="text-button" data-refund="${escapeHtml(p.id)}" data-website-payment="${p.websitePayment===true}" data-amount="${Number(p.refundableAmount)}" data-currency="${escapeHtml(p.currency)}">Refund</button>`) : "—"]);
  ...
  const identityRepairPanel = c && !c.user.email && (c.user.providers || []).some((provider) => provider === "google.com" || provider === "apple.com") ? `<section class="panel form-panel"><header><div><p class="section-kicker">IDENTITY REPAIR</p><h3>Restore a missing provider email</h3></div><span class="state-pill">Audit required</span></header><p class="panel-copy">Use only after the player confirms the exact Google or Apple account. The repaired email remains unverified until the player signs in again, and all existing sessions are revoked.</p><form id="email-repair-form" class="stack-form"><label>Confirmed account email<input type="email" name="email" required autocomplete="off"></label><label>Audit reason<textarea name="reason" minlength="10" maxlength="500" required placeholder="Why is this identity repair authorized?"></textarea></label><button class="button primary">Repair email and revoke sessions</button></form></section>` : "";
  const detail = c ? `${c.deletionRequest?.state === "scheduled" ? `<section class="alert warning"><div><strong>Account deletion scheduled</strong><span>Profile and cloud saves will be purged after ${formatDate(c.deletionRequest.deleteAfter)} unless support cancels during recovery.</span></div><button class="button secondary" data-customer-action="cancel-deletion">Cancel deletion</button></section>` : ""}${requestPanel}${identityRepairPanel}<section class="customer-grid"><article class="panel detail-card"><header><div><p class="section-kicker">ACCOUNT</p><h3>${escapeHtml(c.user.email || c.user.uid)}</h3></div><span class="state-pill">${c.user.disabled ? "Disabled" : "Enabled"}</span></header><dl class="definition-grid"><div><dt>Firebase UID</dt><dd>${escapeHtml(c.user.uid)}</dd></div><div><dt>Login providers</dt><dd>${escapeHtml((c.user.providers || []).join(", ") || "None")}</dd></div><div><dt>Email verified</dt><dd>${c.user.emailVerified ? "Yes" : "No"}</dd></div><div><dt>Last sign-in</dt><dd>${formatDate(c.user.lastSignInAt)}</dd></div></dl><div class="card-actions"><button class="button secondary" data-customer-action="sessions">Revoke sessions</button><button class="button danger" data-customer-action="access">${c.user.disabled ? "Enable account" : "Disable account"}</button></div></article>
...
function renderOperations(data) {
  ...
  const emailRows=(emails.deliveries||[]).map(d=>[formatDate(d.startedAt),d.state,d.locale,d.messageId]);
  ...
  <section class="panel"><header><div><p class="section-kicker">PURCHASE EMAILS</p><h3>orders@wonderlang.app</h3></div></header><p class="panel-copy">Sending: ${emails.enabled?'enabled':'disabled'} · SMTP credential: ${emails.configured?'configured':'missing'} · Start: ${formatDate(emails.startAt)}. Check Workspace email logs before retrying an uncertain delivery.</p><button class="button secondary" data-test-order-email>Send a test to jonathan@wonderlang.app</button>${table(['Started','State','Language','Message ID'],emailRows)}</section>
  <section class="panel spaced"><header><div><p class="section-kicker">SUBSCRIPTION RECONCILIATION</p><h3>Missed-webhook safety</h3></div></header>${table(["Started", "State", "Checked", "Succeeded", "Failed", "Provider access"], reconciliation)}</section>
...
function renderInventory(data) {
  ...
  return `${pageIntro("KEY INVENTORY", "Google Sheets key registry", "Live counts from the Google Sheets registry used for key delivery. This page never changes or allocates keys.", '<button class="button secondary" data-view="inventory">Refresh inventory</button>')}
  <p class="panel-copy">${ready ? "Checked" : "Check attempted"}: ${formatDate(data.checkedAt)} · Read-only Google Sheets access</p>
  ${!ready ? `<aside class="alert warning"><div><strong>Google Sheets inventory is unavailable</strong><span>${escapeHtml((data.issues || []).join(" "))}</span></div></aside>` : `
  <section class="metrics-grid">${metric("Available", data.totals?.available || 0, "Unassigned key rows")}${metric("Assigned", data.totals?.assigned || 0, "Assigned key rows")}${metric("Total", data.totals?.total || 0, "Nonempty key rows")}</section>
  ${(data.issues || []).map(issue => `<aside class="alert warning"><p>${escapeHtml(issue)}</p></aside>`).join("")}
  <section class="inventory-grid">${(data.summary || []).map(r => `<article class="inventory-card ${r.lowStock ? "low" : ""}"><p>${escapeHtml(r.sheetTab)}</p><strong>${Number(r.available).toLocaleString()}</strong><span>available</span><small>${Number(r.assigned).toLocaleString()} assigned · ${Number(r.total ?? (r.available + r.assigned)).toLocaleString()} total</small><small>Low-stock alert at ${Number(r.lowStockThreshold).toLocaleString()}${r.duplicateRows ? ` · ${Number(r.duplicateRows)} duplicate rows` : ""}</small></article>`).join("")}</section>`}
  <section class="panel"><header><h3>How counts are calculated</h3></header><p class="panel-copy">Each configured inventory tab is read directly from Google Sheets. Rows with a key in column A and an empty assignment cell in column B are available; rows with an assignment are counted as assigned. Empty key rows are ignored. Keys and customer emails stay on the server and are never displayed here. Steam and Itch keys are separate from account entitlements.</p></section>`;
}
...
function bindView() {
  document.querySelector('[data-test-order-email]')?.addEventListener('click',async(event)=>{const button=event.currentTarget;button.disabled=true;try{await mutate('/admin-api/v1/order-emails/test',{},'Test accepted by Google. Check jonathan@wonderlang.app for delivery.');}finally{button.disabled=false;}});
  ...
  document.querySelector("#email-repair-form")?.addEventListener("submit", async (event) => { event.preventDefault(); const f = new FormData(event.currentTarget); const body = { email: f.get("email"), reason: f.get("reason") }; if (await mutate(`/admin-api/v1/customers/${state.customer.user.uid}/repair-email`, body, "Email repaired. Existing sessions were revoked; the player must sign in again.")) { state.customer = await api(`/admin-api/v1/customers/${state.customer.user.uid}`); loadView("customers"); } });
...
function signInScreen(message = "Sign in with an approved WonderLang administrator account.") {
  ...
  document.querySelector('[data-provider="google"]')?.addEventListener("click", () => { const provider = new GoogleAuthProvider(); provider.addScope("email"); provider.addScope("profile"); providerSignIn(provider); });
