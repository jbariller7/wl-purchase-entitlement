...
function render(){
 ...
 function update(){const params=new URLSearchParams({offer,lang,currency,...options});if(/^[a-f0-9]{64}$/.test(query.get('experimentToken')||''))params.set('experimentToken',query.get('experimentToken'));if(campaign)params.set('campaign',campaign.id);addAttribution(params,metaContext);for(const key of ['ttclid','gclid','gbraid','wbraid']){const value=query.get(key);if(value&&value.length<=255)params.set(key,value)}buy.href='/shop/checkout/?'+params}
