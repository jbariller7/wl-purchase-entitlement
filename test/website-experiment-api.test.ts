...
const calls=vi.hoisted(()=>({enroll:vi.fn(),record:vi.fn(),withdraw:vi.fn(),limit:vi.fn()}));
vi.mock('../src/infrastructure/firebase.js',()=>({firestore:()=>({})}));
vi.mock('../src/http/rate-limit.js',()=>({consumeRateLimit:calls.limit}));
vi.mock('../src/analytics/website-experiments.js',async importActual=>({...await importActual<any>(),enroll:calls.enroll,recordExperiment:calls.record,withdraw:calls.withdraw}));
