...
describe("cloud storage monitoring", () => {
  it("counts immutable and staging bytes and flags only stale staging objects", () => {
    const snapshot = summarizeCloudStorage({
      revisions: [
        { path: "cloud-saves/a/1.json", size: 1000, updatedAt: "2026-08-22T12:00:00.000Z" },
        { path: "cloud-saves/a/2.json", size: 2000, updatedAt: "2026-08-24T11:00:00.000Z" }
      ],
      staging: [
        { path: "cloud-save-uploads/a/fresh.json", size: 300, updatedAt: "2026-08-24T11:00:00.000Z" },
        { path: "cloud-save-uploads/a/stale.json", size: 400, updatedAt: "2026-08-22T11:00:00.000Z" }
      ],
  ...
  it("lists both server-only prefixes and persists a dated snapshot", async () => {
    const prefixes: string[] = [];
    const source: CloudStorageInventorySource = {
      list: async (prefix) => {
        prefixes.push(prefix);
        return prefix === "cloud-saves/"
  ...
  it("fails closed on malformed provider size metadata", () => {
    expect(() => summarizeCloudStorage({
      revisions: [{ path: "cloud-saves/a/bad.json", size: Number.NaN, updatedAt: now.toISOString() }],
      staging: [],
      now,
  ...
  it("records only a generic failure state when inventory listing fails", async () => {
    let failures = 0;
    const source: CloudStorageInventorySource = { list: async () => { throw new Error("cloud-saves/private-uid/secret-path"); } };
    const repository: CloudStorageMetricsRepository = {
      previous: async () => undefined,
