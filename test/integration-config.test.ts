...
describe("isolated integration configuration", () => {
  ...
  it("publishes a host-restricted, side-effect-free account UI demo", () => {
    ...
    expect(widget).not.toContain('this.request("/api/v1/checkout"');
    expect(widget).toContain('href="https://wonderlang.net/"');
  ...
  it("binds PC/Mac sign-in to an automatic high-entropy browser handoff", () => {
    ...
    expect(account).not.toContain('data-action="discounted-premium"');
    expect(account).not.toContain('data-action="monthly"');
    expect(account).not.toContain('data-action="polyglot"');
    expect(account).toContain('class="wl-website-link"');
  ...
  it("keeps the Android migration server-authoritative and subscription-aware", () => {
    ...
    expect(activity).toContain('"wonderlangmonthly" -> "WonderLang Mobile Monthly"');
    expect(activity).toContain('"wonderlangfull" -> "WonderLang Polyglot Permanent"');
    expect(activity).toContain('"wonderlangmonthly" -> "subscription"');
    ...
    expect(manager).toContain('"wonderlangmonthly" -> premiumLifetime || subscriptionState in setOf("active", "grace")');
    expect(manager).toContain('"wonderlangfull" -> premiumLifetime || hasAndroidPermanent || accessKind == "legacy"');
    expect(manager).toContain('if (!fullGameEntitled || !entitlements.optBoolean("fullGame", false)) return false');
  ...
  it("keeps Android offline access account-bound, encrypted, and time-limited", () => {
    ...
    expect(manager).toContain("nowWallMs < fullGameLeaseExpiresAtWallMs");
    expect(manager).toContain("nowWallMs < cloudSaveLeaseExpiresAtWallMs");
    expect(manager).toContain("subscriptionProviderExpiry(entitlements)?.let { expiry = minOf(expiry, it) }");
    ...
    expect(manager).toContain("fullGameLeaseExpiresAtWallMs");
    expect(manager).toContain("cloudSaveLeaseExpiresAtWallMs");
    expect(manager).toContain("entitlementVerifiedAtElapsedMs");
    expect(manager).toContain("trustedAgeMs <= fullGameLeaseMaximumAgeMs");
    expect(manager).toContain("trustedAgeMs <= cloudSaveLeaseMaximumAgeMs");
    expect(manager).toMatch(/fun getAccountSnapshot\(\): String \{[\s\S]*?refreshRuntimeLeaseState\(\)[\s\S]*?return cachedAccountJson/);
    expect(manager).toMatch(/private fun refreshRuntimeLeaseState\(\): Boolean \{[\s\S]*?offlineCloudSaveExpired[\s\S]*?cachedAccountJson/);
    expect(manager).toContain("offlineFullGameLeaseMaximumAge");
    ...
    expect(rmmz).toContain('provider === "google_play"');
    expect(rmmz).toContain("https://play.google.com/store/account/subscriptions?sku=wonderlangmonthly&package=com.wonderlang.app");
    expect(rmmz).toContain('provider === "apple"');
    expect(rmmz).toContain('provider !== "stripe"');
