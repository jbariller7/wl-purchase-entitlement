...
- All fulfillment, advertising, webhook-processing, cancellation, deletion-processing, outbox, Stripe-mutation, subscription-reconciliation and cloud-storage-monitoring switches remain disabled.
...
- Published cloud-storage-monitoring build: `5ce42faef01d7d9fa6c97f9289a05d4cb5f9e143` (Netlify deploy `6a8cb7b5e2c5450009bc6cfe`; feature commit `ead6460aba4834ab45681c52a39c327174f340aa`).
- `APP_ENVIRONMENT=test` in every deploy context.
- Stripe, Google Play and Apple webhook processing; the async worker; ad conversions; legacy fulfillment; subscription cancellation; account-deletion processing; Stripe mutations; scheduled subscription reconciliation; and aggregate cloud-storage monitoring are all `false` in every deploy context.
