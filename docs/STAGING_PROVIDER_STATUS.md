...
- All fulfillment, advertising, webhook-processing, cancellation, deletion-processing, outbox, Stripe-mutation and subscription-reconciliation switches remain disabled.
...
- Published reconciliation build: `549ec084c3435fc73129da4850fc278d94f98ba3` (Netlify deploy `6a8cae4e904fc400089a832c`).
- `APP_ENVIRONMENT=test` in every deploy context.
- Stripe, Google Play and Apple webhook processing; the async worker; ad conversions; legacy fulfillment; subscription cancellation; account-deletion processing; Stripe mutations; and scheduled subscription reconciliation are all `false` in every deploy context.
