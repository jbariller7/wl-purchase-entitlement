...
- Current published Git baseline before the next UI candidate is `6bf6c5a`. The published cloud-save retention/cleanup baseline is `580511e` (Netlify deploy `6a8ce2733483ab0008e01166`), the secure NW.js device-sign-in baseline is `4f62c63`, and the earlier honest-UI baseline was `cfd177b`.
...
- Browser smoke testing passed for the `/` redirect, all eight `/admin/` sections, `/account/?demo=1`, `/setup/`, and `/rmmz-test/`. The admin page never silently substitutes demo records when configuration is missing; it displays the real configuration gate. Simulated data requires an explicit `?demo=1` and both customer/admin demos display prominent non-live warnings. The latest local UI candidate was exercised interactively: customer lookup; administrator grant/revoke; account disable/re-enable; session and deletion controls; partial refund with typed confirmation; regional Stripe price replacement while preserving USD and other currencies; CSV import; outbox, cloud-save-cleanup and provider-event retries; alert routing; Google demo sign-in; PC/Mac code preview/approval/removal; provider-specific subscription management; monthly/Polyglot/Premium checkout gates; and exact-phrase account deletion all produced the intended state transitions. Every rendered customer action/form and every administrator operation family now has an executable client-to-protected-route contract test, including loading, error, empty, permission, disabled, success and explicit-demo states. Admin tables escape all provider/customer strings and permit markup only through an internal symbol-branded cell. Desktop and 390-pixel account/admin layouts have no page-level horizontal overflow. Admin Operations renders scheduled-reconciliation history, aggregate token-vault key counts and aggregate Cloud Storage metrics without tokens, player IDs or object paths.
